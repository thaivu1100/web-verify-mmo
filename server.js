// Hệ thống Web Verify MMO - Dùng PostgreSQL, đồng bộ token, log chi tiết
...
if (!process.env.DATABASE_URL) {
    console.error('❌ THIẾU BIẾN MÔI TRƯỜNG DATABASE_URL! Vui lòng thêm vào Render.');
    process.exit(1);
}
...
// API tạo token (có xử lý lỗi chi tiết)
app.post('/api/create-token', async (req, res) => {
    try {
        ...
        await pool.query(
            `INSERT INTO active_tokens (token, user_id, task_type, ip, fingerprint, created_at)
             VALUES ($1, $2, $3, $4, $5, $6)
             ON CONFLICT (token) DO UPDATE SET user_id = EXCLUDED.user_id, task_type = EXCLUDED.task_type, ip = EXCLUDED.ip, fingerprint = EXCLUDED.fingerprint, created_at = EXCLUDED.created_at`,
...
app.get('/verify/:token', async (req, res) => {
    ...
    try {
        ...
        res.status(500).send(`<!DOCTYPE html><html><head><title>LỖI HỆ THỐNG</title><style>body{font-family:Arial;background:linear-gradient(135deg,#ff4757,#c0392b);display:flex;justify-content:center;align-items:center;min-height:100vh;}.container{background:#fff;padding:40px;border-radius:20px;text-align:center;}h2{color:#c0392b;}</style></head><body><div class=container><h2>⚠️ LỖI HỆ THỐNG</h2><p>Vui lòng thử lại sau ít phút.</p><a href="https://t.me/Vuotlinkcaytienbot">🤖 Quay lại Bot</a></div></body></html>`);
    }
});
