...
// ========== API tạo token (KHÔNG kiểm tra blacklist) ==========
app.post('/api/create-token', (req, res) => {
    ...
    // Bỏ qua blacklist – vì đây là bot, không cần chặn IP
    const tokens = readJSON(TOKENS_FILE);
    tokens[token] = {
...
// Trang xác minh – kiểm tra blacklist đầy đủ
app.get('/verify/:token', async (req, res) => {
    ...
    // Blacklist (chỉ chặn người dùng cuối)
