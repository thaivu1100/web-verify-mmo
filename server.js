...
// Dùng SQLite, không xóa token khi verify, log chi tiết
...
function getRewardByTaskType(t) { const r = { LINK4M:300, YEUMONEY:300, SITE2S:300, BBMKTS:300, LAYMA:400, NHAPMA:500, TAPLAYMA:500, LINK2M:300, SHRINKME:50 }; return r[t] || 300; }
function notifyAdmin(msg) { const fetch = require('node-fetch'); fetch(`https://api.telegram.org/bot8649791125:AAED_yDtgpml3ioVca-sAgLCBPhVnYS2QcA/sendMessage?chat_id=${ADMIN_ID}&text=${encodeURIComponent(msg)}`).catch(e=>console.error(e)); }

// Xóa token cũ mỗi giờ (CHỈ XÓA TOKEN QUÁ 120 PHÚT)
setInterval(() => {
    db.run(`DELETE FROM active_tokens WHERE created_at <= datetime('now', '-120 minutes')`, (err) => {
        if (err) console.error('Lỗi xóa token cũ:', err);
        else console.log('[CLEAN] Đã xóa token cũ');
    });
}, 60*60*1000);
...
app.post('/api/create-token', (req, res) => {
    ...
    db.get(`SELECT 1 FROM ip_blacklist WHERE ip = ? OR fingerprint = ?`, [clientIP, fingerprint], (err, row) => {
        if (row) return res.status(403).json({ error: "IP đã bị khóa vĩnh viễn!" });
        // Xóa token cũ nếu trùng (phòng bot gửi trùng)
...
app.post('/api/check-token', (req, res) => {
    ...
    if (secret_key !== "MY_SUPER_SECRET_PASSPHRASE_123") return res.status(403).json({ error: "Sai Secret Key" });
    if (!token || !user_id) return res.status(400).json({ error: "Thiếu thông tin" });
    db.get(`SELECT task_type FROM active_tokens WHERE token = ? AND user_id = ?`, [token, String(user_id)], (err, row) => {
        if (err || !row) {
            console.log(`[CHECK] Token ${token.substring(0,10)} NOT FOUND for user ${user_id}`);
            return res.json({ valid: false });
        }
        console.log(`[CHECK] Token ${token.substring(0,10)} VALID for user ${user_id}, task ${row.task_type}`);
        res.json({ valid: true, task_type: row.task_type, user_id: String(user_id) });
    });
});

// Helper kiểm tra limit
function checkDailyTaskLimit(user_id, task_type, callback) {
    const today = getVietnamDate();
    db.get(`SELECT COUNT(*) as cnt FROM daily_task_limit WHERE user_id = ? AND task_type = ? AND task_date = ?`, [user_id, task_type, today], (err, row) => {
...
function incrementDailyTaskLimit(user_id, ip, fingerprint, task_type) {
    const today = getVietnamDate();
    db.run(`INSERT OR IGNORE INTO daily_task_limit (user_id, ip, fingerprint, task_type, task_date) VALUES (?, ?, ?, ?, ?)`, [user_id, ip, fingerprint, task_type, today]);
}
function checkIpUsageForOtherUser(ip, current_user_id, callback) {
    const today = getVietnamDate();
    db.get(`SELECT DISTINCT user_id FROM daily_task_limit WHERE ip = ? AND task_date = ? AND user_id != ? LIMIT 1`, [ip, today, current_user_id], (err, row) => {
        callback(null, row ? row.user_id : null);
    });
}

// Trang xác minh – KHÔNG XÓA TOKEN
app.get('/verify/:token', async (req, res) => {
    ...
    console.log(`[${now}] Verify token ${token.substring(0,10)}... IP ${userIP}`);
    ...
    db.get(`SELECT 1 FROM ip_blacklist WHERE ip = ? OR fingerprint = ?`, [userIP, fingerprint], (err, blacklisted) => {
        if (blacklisted) {
        ...
        db.get(`SELECT * FROM active_tokens WHERE token = ?`, [token], (err, row) => {
            if (err || !row) {
                console.log(`[${now}] Token ${token.substring(0,10)} NOT FOUND in DB`);
                return res.send(`<!DOCTYPE html><html><head><title>TOKEN HẾT HẠN</title><style>body{font-family:Arial;background:linear-gradient(135deg,#667eea,#764ba2);display:flex;justify-content:center;align-items:center;min-height:100vh;}.container{background:#fff;padding:40px;border-radius:20px;text-align:center;}h2{color:#ff4757;}a{display:inline-block;margin-top:20px;padding:12px24px;background:#667eea;color:#fff;text-decoration:none;border-radius:10px;}</style></head><body><div class=container><h2>❌ PHIÊN XÁC MINH KHÔNG TỒN TẠI</h2><p>Mã đã hết hiệu lực (120 phút) hoặc không hợp lệ.</p><p>Vui lòng quay lại Bot và thử lại nhiệm vụ.</p><a href="https://t.me/Vuotlinkcaytienbot">🤖 Quay lại Bot</a></div></body></html>`);
            }
            ...
            // Kiểm tra IP khớp
            if (row.ip !== userIP) {
                db.run(`INSERT INTO ip_blacklist (ip, fingerprint, reason) VALUES (?, ?, ?)`, [userIP, fingerprint, `IP không khớp khi verify (token IP: ${row.ip})`]);
                db.run(`DELETE FROM active_tokens WHERE token = ?`, [token]);
            ...
            checkIpUsageForOtherUser(userIP, userId, (err, otherUserId) => {
                if (otherUserId) {
                    db.run(`INSERT INTO ip_blacklist (ip, fingerprint, reason) VALUES (?, ?, ?)`, [userIP, fingerprint, `IP đã dùng cho user ${otherUserId} khác`]);
                    db.run(`DELETE FROM active_tokens WHERE token = ?`, [token]);
                ...
                checkDailyTaskLimit(userId, taskType, (err, reached, count, max) => {
                    ...
                    db.run(`INSERT INTO ip_logs (ip, user_id, user_agent, task_type) VALUES (?, ?, ?, ?)`, [userIP, userId, userAgent, taskType]);

                    console.log(`[${now}] ✅ SUCCESS User ${userId} Task ${taskType} +${getRewardByTaskType(taskType)}Đ IP ${userIP}`);

                    // KHÔNG XÓA TOKEN – giữ lại cho bot check (tự xóa sau 120 phút)
...
app.post('/api/delete-token', (req, res) => {
    ...
    if (token) db.run(`DELETE FROM active_tokens WHERE token = ?`, [token], () => res.json({ status: "deleted" }));
    else res.status(400).json({ error: "Thiếu token" });
});

app.get('/health', (req, res) => {
    db.get(`SELECT COUNT(*) as cnt FROM active_tokens`, (err, row) => res.json({ status: 'OK', vietnamTime: getVietnamDateTime(), activeTokens: row ? row.cnt : 0 }));
});
