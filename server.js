...
// Chống trùng IP: mỗi IP chỉ được sử dụng bởi 1 user_id duy nhất trong ngày
...
// Lưu IP đã được sử dụng bởi user_id nào trong ngày (chống 1 IP dùng nhiều tài khoản)
const ipUsageToday = new Map(); // key: ip, value: { user_id, date }
...
// Dọn dẹp ipUsageToday mỗi ngày (đơn giản: reset theo ngày)
function cleanIpUsage() {
    const today = getVietnamDate();
    for (const [ip, data] of ipUsageToday.entries()) {
        if (data.date !== today) {
            ipUsageToday.delete(ip);
        }
    }
}
setInterval(cleanIpUsage, 60 * 60 * 1000); // mỗi giờ kiểm tra
...
// API kiểm tra token – BỎ QUA KIỂM TRA IP (chỉ bot gọi)
