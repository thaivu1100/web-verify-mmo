...
// Dùng SQLite để lưu token bền vững, tránh mất token khi restart server
...
// Helper: kiểm tra giới hạn nhiệm vụ (dựa trên bảng daily_task_limit)
function checkDailyLimit(user_id, task_type, callback) {
    ...
    db.get(`SELECT COUNT(*) as cnt FROM daily_task_limit WHERE user_id = ? AND task_type = ? AND task_date = ?`,
        ...
        if (err) return callback(err, false, 0, 0);
...
function incrementDailyLimit(user_id, ip, fingerprint, task_type) {
    const today = getVietnamDate();
...
// Helper: kiểm tra IP đã dùng cho user khác chưa (1 IP/ngày chỉ 1 user)
...
app.post('/api/create-token', (req, res) => {
    ...
    isIPBlacklisted(clientIP, fingerprint, (err, blacklisted) => {
        ...
        // Xóa token cũ (phòng tránh trùng)
...
// API kiểm tra token (dành cho bot) – KHÔNG XÓA TOKEN
...
app.get('/verify/:token', async (req, res) => {
    ...
    isIPBlacklisted(userIP, fingerprint, async (err, blacklisted) => {
        ...
        db.get(`SELECT * FROM active_tokens WHERE token = ?`, [token], async (err, row) => {
            ...
            // Kiểm tra IP có khớp với lúc tạo không
            if (row.ip !== userIP) {
            ...
            checkIpUsageForOtherUser(userIP, userId, (err, otherUserId) => {
                ...
                checkDailyLimit(userId, taskType, (err, reached, count, max) => {
                    if (reached) {
                        db.run(`DELETE FROM active_tokens WHERE token = ?`, [token]);
                    ...
                    // Tăng limit
                    incrementDailyLimit(userId, userIP, fingerprint, taskType);
                    // Ghi log IP
                    ...
                    // KHÔNG XÓA TOKEN – vẫn giữ để bot kịp check
